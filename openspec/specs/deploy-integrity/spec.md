# deploy-integrity Specification

## Purpose

What deploy.sh must prove before a gateway is left running new files:
that every executable a shipped unit invokes actually landed, that the
kernel object matches the one that was built and rehearsed, that
verification happens before installation rather than after it, and that
a rollback has a coherent object and spec pair to roll back to.
