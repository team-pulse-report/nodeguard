...
The automated verification the repository carries for its userspace
control plane, and the gate it runs behind. The suite is hermetic by
requirement (standard library only, no root, no network, no bpftool) so
that the logic which decides what gets blocked is exercised on any
workstation, not only inside the build container or in production.
