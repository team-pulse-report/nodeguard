...
How Suricata is configured, fed, and kept current as the detection half of
the system, given that it detects passively and never drops a packet
itself. Covers the generated per-host configuration, the memory caps the
hosts actually sustain, the daily ruleset update and its non-blocking
reload, and the eve.json contract the responder consumes. Detection
quality here is an input to enforcement, so its freshness is a
