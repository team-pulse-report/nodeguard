...
The properties that keep nodeguard's own machinery from failing
silently: every timer-driven run is time-bounded so a hang cannot halt
detection forever, and every attach-state reader honors the
three-valued contract (attached, detached, unknown) so a broken
xdp-loader is never reported as a clean datapath.
