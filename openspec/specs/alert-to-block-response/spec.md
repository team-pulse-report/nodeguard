...
The rules governing when a Suricata alert may become a kernel block, which
is the only path by which nodeguard blocks anything automatically. The
gates here exist because an alert is attacker-influenced input: a source
address can be spoofed, so evidence of a completed bidirectional TCP flow
is required before any address is written, and UDP or ICMP alerts stay
log-only unless a signature is promoted by hand. Rate caps, TTL
escalation, and the journal that records what was blocked and why are all
