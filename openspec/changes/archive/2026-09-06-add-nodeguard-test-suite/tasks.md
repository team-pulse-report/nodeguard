...
- [x] 1.1 Write tests/ngtest.py part 1 (loaders): `load_bin(
...
- [x] 1.2 Write tests/ngtest.py part 2 (fakes): shared in-memory fake
...
- [x] 1.3 Write tests/test_ngmap.py part 1: key_bytes/decode_key and
      ...
      allowlisted range
- [x] 1.4 Write tests/test_ngmap.py part 2: cmd_block refusal ladder
...
- [x] 1.5 K3 failing-first regressions: set-config with negative slot,
...
- [x] 1.6 Implement the K3 fix in bin/ngmap.py: range checks in
...
- [x] 2.1 Write tests/test_feeds.py part 1 (parsers): valid Spamhaus
...
- [x] 2.2 Implement the S4 parser layer in bin/nodeguard-feeds:
...
- [x] 2.3 Implement the S4 run layer: add a writes counter incremented
...
- [x] 2.4 Write tests/test_feeds.py part 2 (run-level S4
...
- [x] 2.5 Write tests/test_feeds.py part 3 (feed gates), on the same
...
- [x] 3.1 Behavior-preserving extraction in bin/nodeguard-responder:
...
- [x] 3.2 Write tests/test_responder.py part 1 (the seven gates): one
...
- [x] 3.3 Write tests/test_responder.py part 2: TTL escalation doubles
...
- [x] 4.1 Add the env seam in bin/nodeguard-watchdog: KV, STATE, and
...
- [x] 4.2 Write tests/test_watchdog_anom.py using the harness
...
- [x] 5.1 Wire the suite into build/build.sh as a `== unit tests ==`
...
- [x] 5.2 Full local verification gate: `python3 -m unittest discover
      ...
      add-nodeguard-test-suite --strict` passes. The container build and
      the netns rehearsal are unrunnable on the macOS arm64 development
      machine (no privileged Fedora container); they stay pending a
      Fedora build host and are not gated here
- [x] 5.3 Record the new tree and the new gate in the repository
      documentation: a `tests/` line in README.md's Layout block, a
      `tests/` entry plus the unit-test gate in docs/code-map.md's
      build/build.sh description, and in docs/design.md the build gate
      in sections 5 and 10 plus the four `build/build.sh` line
      citations the gate shifted (`:23` to `:29`, `:101` to `:107`)
